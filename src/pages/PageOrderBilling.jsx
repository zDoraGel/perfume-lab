...
// ราคาเต็มตามขนาดขวด (fix ไว้ก่อน — ถ้าจะเปลี่ยนราคาทีหลังแก้ตรงนี้จุดเดียว)
const BOTTLE_SIZE_PRICES = { 5: 159, 15: 469, 30: 859 } // 50ml, 100ml ยังไม่ fix ราคา — กรอกเองตามปกติ
...
function NewOrderForm({ formulas, customers, orders, onSaved, onOrderSaved, onViewHistory }) {
  ...
  const [items,           setItems]           = useState([
    { formula_id:'', bottle_ml:15, qty:1, unit_price:String(BOTTLE_SIZE_PRICES[15]), original_price:'', discount_type:null, discount_value:'', discount_reason:'' }
  ])
  ...
  function addItem() {
    setItems(p => [...p, { formula_id:'', bottle_ml:15, qty:1, unit_price:String(BOTTLE_SIZE_PRICES[15]), original_price:'', discount_type:null, discount_value:'', discount_reason:'' }])
  }
  ...
  function updateItem(i, field, value) {
    setItems(p => p.map((it, idx) => {
      ...
      // เปลี่ยนขนาดขวด → เติมราคาเต็มตามที่ fix ไว้ให้อัตโนมัติ (ถ้ามีราคากำหนดไว้สำหรับขนาดนั้น)
      if (field === 'bottle_ml' && BOTTLE_SIZE_PRICES[Number(value)] != null) {
        updated.unit_price = String(BOTTLE_SIZE_PRICES[Number(value)])
        updated.original_price = '' // เคลียร์ราคาลดเดิม ให้เริ่มนับจากราคาเต็มใหม่
      }
      return updated
    }))
  }
    ...
    <div style={{ background:S.white, borderRadius:14, border:`1px solid ${S.border}`,
      ...
      <div style={{ marginBottom:14, padding:'10px 12px', background:S.bg, borderRadius:8 }}>
        <label style={{ display:'flex', alignItems:'center', gap:8,
          cursor: redeemType === 'free_shipping' ? 'not-allowed' : 'pointer',
          fontSize:13, color:S.ink }}>
          <input type="checkbox"
            checked={redeemType === 'free_shipping' ? true : hasShipping}
            disabled={redeemType === 'free_shipping'}
            onChange={e => setHasShipping(e.target.checked)}/>
          ลูกค้าต้องเสียค่าส่ง
        </label>
        {redeemType === 'free_shipping' && (
          <div style={{ fontSize:11, color:S.green, marginTop:4 }}>
            🎁 บังคับติ๊กไว้ — ลูกค้าแลกแต้มส่งฟรี (ยกเลิกการแลกก่อนถ้าจะแก้)
          </div>
        )}
        {redeemType !== 'free_shipping' && hasShipping && (
