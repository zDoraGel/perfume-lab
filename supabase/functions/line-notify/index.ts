...
Deno.serve(async (_req) => {
  ...
  const { data: batches } = await supabase
    ...
    .select("id, formula_id, produced_at, qty_produced, qty_sold, formulas(name)");
  ...
  if (batches) {
    for (const b of batches) {
      ...
      if (daysSince === 30) {
        messages.push(`🌿 Aging ครบ 30 วันแล้ว!\n${name}\nพร้อมขาย ${remaining} ขวด`);
      } else if (daysSince === 29) {
        messages.push(`⏳ พรุ่งนี้ครบ Aging!\n${name}`);
      }
    }
  }
  ...
  const fullMessage = `🌿 Linen Theory\n${today.toLocaleDateString("th-TH")}\n${"─".repeat(20)}\n\n` + messages.join("\n\n─────────\n\n");
